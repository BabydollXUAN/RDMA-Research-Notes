...
1. **流量分类与分块 (Segmentation & Coding)**
    ...
    - **操作：** 将消息分割为多个Block。每个Block包含 $n$ 个包，其中包括 $x$ 个数据包和 $y$ 个校验包（Parity Packets）1111。
        
    - _逻辑：_ 只要接收端收到任意 $x$ 个包即可还原数据，无需重传。
        
2. **子流负载均衡 (UnoLB - Subflow Assignment)**
    
    - **分配：** 将同一个Block内的 $n$ 个数据包分散分配到多个子流（Subflows）上 222。
        
    - **路由：** 每个子流通过改变包头（如源端口或IPv6 Flow Label）来利用ECMP哈希到不同的物理路径上 3。
        
    - **故障规避：** 如果收到NACK或发生超时（Timeout），UnoRC会立即将该子流“重路由”（Re-route），从已知健康的子流池中随机选择一个新的路径，避开故障链路 4。
...
3. **双队列维护 (Queue Management)**
    ...
    - **幽灵计数：** 同时，交换机在逻辑上维护一个虚拟的**幽灵队列（Phantom Queue）**计数器。物理队列每进一个包，幽灵队列计数器也增加 5。
        
4. **差异化排空 (Differential Draining)**
    
    - **物理队列：** 以链路的全线速（Line Rate, e.g., 100Gbps）排空。
        
    - **幽灵队列：** 以略低于线速的速率（例如 90% Line Rate）进行逻辑排空 6。
...
5. **ECN 标记 (Marking Logic)**
    ...
    - **操作：** 如果任意一个队列超过阈值，则在数据包的IP头中标记 ECN（Congestion Experienced）位 7。
...
6. **解码与重组 (Reconstruction)**
    ...
    - **还原：** 如果在计时器内收到 $\ge x$ 个包，直接使用MDS算法还原原始数据块 8。
        
    - **异常处理：** 如果超时或包数量不足，发送 **NACK** 给发送端请求重传 9。
...
8. 状态判断与窗口调整 (State Machine)
    ...
    - **状态 B：普通拥塞 (Congested)**
        ...
        - _区分真假拥塞：_ UnoCC检查数据包的往返延迟（Delay）：
            ...
            - **情形1（幽灵拥塞）：** 有ECN标记，但延迟极低（接近基准RTT）。说明是幽灵队列满了，物理队列还是空的。
                ...
                - _动作：_ **温和减速**。将减小因子乘以0.3（$MD_{scale} \times 0.3$）11。
                    
            - **情形2（物理拥塞）：** 有ECN标记，且延迟较高。
                
                - _动作：_ **全额减速**。执行标准的MD操作 12。
    ...
    - **状态 C：极端拥塞 (Extremely Congested)**
        ...
        - _操作 (Quick Adapt - QA)：_ **急刹车**。不等待MD，直接将 $cwnd$ 重置为当前实际的 `bytes_acked` 值 13。
