...
UnoLB 觉得“把鸡蛋放在一个篮子里”太蠢了。它决定采用**“子流” (Subflow)** 策略。利用现代网络的多路径特性，把大流量拆散，并行传输 。结合了 **纠删码 (Erasure Coding)** 和 **动态重路由**。
...
现在，你理解为什么代码里会有 `subflow[index]` 和 `update_subflow(packet)` 这种逻辑了吗？12 这就是那个“给车队换路线”的过程。