...
Uno 系统将拥塞控制、负载均衡和抗丢包能力集成，为数据中心内部和数据中心之间的通信提供了统一的解决方案。核心是UnoCC(Congestion Control)和UnoRC(Reliable Connectivity)。
当消息传输速率受限，采用UnoCC，解决公平性、收敛速度和 BDP 不匹配问题。
大多数消息在跨数据中心广域网传输时都受延迟限制时，采取UnoRC。
Uno 方法全流程概览
1.发送端(UnoRC): 编码与分流 消息切片->添加纠删码 校验包->将 EC 块打散到多个子流/路径。
2.网络交换机：幽灵队列与标记 数据包进入物理队列，同时幽灵队列计数器增加-> 幽灵队列“满”时，提前标记ECN。
3.接收端：解码与反馈 集齐足够的数据/校验包  ->立即解码还原->回复 ACK (携带ECN 标记信  息 ) 。
4.发送端(UnoCC):状态判断与速率调整 根据ACK中的ECN和延迟信息，分为不拥塞、拥塞和极端拥塞三种情况。前两种采用AIMD调整、第三种采用Quick Adapt策略调整。基于 ACK/NACK 状态动态重路由，实现对“坏路径”的精准规避 。
